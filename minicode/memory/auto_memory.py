...
def get_auto_mem_path(project_root: str) -> str:
    """返回项目级记忆目录路径：<projectRoot>/.minicode/memory/。

    保留尾部分隔符，确保前缀匹配不会误命中类似 memoryxyz 的路径。
    支持 minicode_REMOTE_MEMORY_DIR 环境变量覆盖。
    """
    override = os.environ.get("minicode_REMOTE_MEMORY_DIR", "")
    if override:
        return override.rstrip(os.sep) + os.sep
    abs_root = os.path.abspath(project_root)
    return os.path.join(abs_root, ".minicode", "memory") + os.sep


def get_user_auto_mem_path() -> str:
    """返回用户级记忆目录路径：~/.minicode/memory/。
    ...
    if not home:
        return ""
    return os.path.join(home, ".minicode", "memory") + os.sep
...
class MemoryManager:
    ...
    def __init__(self, project_root: str) -> None:
        ...
        # 用户级：~/.minicode/memory/ — user/feedback 类型记忆
        self._user_mem_dir = get_user_auto_mem_path()
        # 项目级：<projectRoot>/.minicode/memory/ — project/reference 类型记忆
    ...
    def user_path(self) -> Path:
        ...
        return Path.home() / ".minicode" / "memory" / ENTRYPOINT_NAME
    ...
    def user_mem_dir(self) -> Path:
        """用户级记忆目录（~/.minicode/memory/）。"""
        return Path(self._user_mem_dir.rstrip(os.sep)) if self._user_mem_dir else Path.home() / ".minicode" / "memory"

    @property
    def project_mem_dir(self) -> Path:
        """项目级记忆目录（<project>/.minicode/memory/）。"""

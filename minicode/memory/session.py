    ...
    def from_message(cls, message: Message) -> list[SessionRecord]:
        ...
        if message.tool_results:
            ...
            if message.tool_uses:
                ...
                records.append(
                    cls(type=RecordType.ASSISTANT,
                        content=content_blocks, timestamp=now)
                )
            else:
                records.append(
                    cls(type=RecordType.ASSISTANT,
                        content=message.content, timestamp=now)
                )
        else:
            records.append(
    ...
    for record in records:
        ...
        if record.type == RecordType.COMPACT_BOUNDARY:
            ...
            messages.append(Message(
                role="user", content="本次会话延续自之前的对话，因上下文空间不足进行了压缩。以下是早期对话的摘要：\n\n" + summary))
...
async def generate_session_summary(
    ...
    summary_conv.history = [
        Message(role="user", content=SESSION_SUMMARY_PROMPT)]
    for msg in recent:

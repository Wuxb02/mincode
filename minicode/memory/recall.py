...
SELECTOR_SYSTEM_PROMPT = (
    "You are selecting memories that will be useful to minicode as it processes "
    ...
    "minicode as it processes the user's query (up to 5). Only include memories "
    ...
    "that are usage reference or API documentation for those tools (minicode is "
...
def format_memory_manifest(memories: list[MemoryHeader]) -> str:
    ...
    for m in memories:
        ...
        if m.description:
            lines.append(
                f"- {scope_tag}{type_tag}{path} ({ts}): {m.description}")
        else:
    ...
    for fn in selected_filenames:
        ...
        if m is not None:
            result.append(RelevantMemory(
                path=m.file_path, mtime_ms=m.mtime_ms))
    return result
...
def render_reminder(memories: list[RelevantMemory]) -> str:
    ...
    for mem in memories:
        ...
        parts.append(
            f"## Memory: {basename} (saved {memory_age(mem.mtime_ms)})\n")

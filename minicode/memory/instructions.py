...
def load_instructions(project_root: str) -> str:
    ...
    发现顺序（低优先级在前，高优先级在后）：
    1. 用户全局：~/.minicode/minicode.md, ~/.minicode/AGENTS.md
    2. 项目目录链：从 git root 到 workDir，每个目录的 minicode.md 和 AGENTS.md
    3. workDir/.minicode/INSTRUCTIONS.md（遗留格式）
    4. workDir/minicode.local.md（本地覆盖）
    """
    ...
    def _add(path: Path) -> None:
        ...
        content = process_includes(
            data, abs_path.parent, root, 0, include_seen)
    ...
    # 1. 用户全局
    _add(home / ".minicode" / "minicode.md")
    _add(home / ".minicode" / "AGENTS.md")
    ...
    # 3. 遗留格式
    _add(root / ".minicode" / "INSTRUCTIONS.md")

    # 4. 本地覆盖
    _add(root / "minicode.local.md")

...
def load_config(path: Path | None = None) -> AppConfig:
    ...
    home = Path.home()
    candidates = [
        home / ".minicode" / "config.yaml",
        cwd / ".minicode" / "config.yaml",
        cwd / ".minicode" / "config.local.yaml",
    ]
    ...
    if merged is None:
        raise ConfigError(
            "No config file found. Expected .minicode/config.yaml "
            "in project or ~/.minicode/config.yaml"
        )
    return merged

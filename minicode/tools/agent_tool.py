...
class AgentTool(Tool):
    ...
    async def execute(self, params: BaseModel) -> ToolResult:
        ...
        from minicode.agents.fork import ForkError, build_forked_messages
        from minicode.agents.parser import AgentDef
        from minicode.agents.tool_filter import resolve_agent_tools
        from minicode.agent import Agent as AgentClass
        ...
        _base_registry = getattr(
            self._parent_agent, '_full_registry', None) or self._parent_agent.registry
        ...
        if p.subagent_type is None:
            from minicode.context import clone_replacement_state
    ...
    async def _execute_as_teammate(self, p: AgentToolParams) -> ToolResult:
        ...
        from minicode.agents.fork import ForkError, build_forked_messages
        from minicode.agents.parser import AgentDef
        from minicode.agents.tool_filter import build_teammate_tools
        from minicode.agent import Agent as AgentClass
        from minicode.conversation import ConversationManager
        from minicode.permissions import (
            DangerousCommandDetector,
        ...
        from minicode.teams.models import BackendType, TeammateInfo
        ...
        _has_full = getattr(self._parent_agent,
                            '_full_registry', None) is not None
        full_registry = getattr(
            self._parent_agent, '_full_registry', None) or self._parent_agent.registry
        ...
        log.info("[teammate] result_tools=%d names=%s",
                 len(_tm_tools), _tm_tools)
    ...
    def _spawn_pane_teammate(
        ...
        try:
            if backend == BackendType.TMUX:
                from minicode.teams.spawn_tmux import spawn_tmux_teammate
                ...
                self._team_manager.register_pane_id(
                    agent_id, pane_info.pane_id)
            elif backend == BackendType.ITERM2:
                from minicode.teams.spawn_iterm2 import spawn_iterm2_teammate
    ...
    async def _execute_with_worktree(self, p: AgentToolParams) -> ToolResult:
        ...
        from minicode.worktree.integration import (
    ...
    def _create_client_for_model(self, model_alias: str) -> LLMClient | None:
        ...
        from minicode.client import create_client
        from minicode.config import ProviderConfig

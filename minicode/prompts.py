...
class PromptBuilder:
    ...
    def build(self) -> str:
        ...
        parts = [s.content.strip()
                 for s in self._sections if s.content.strip()]
        return "\n\n".join(parts)
...
def build_plan_mode_exit_reminder(plan_path: str, plan_exists: bool) -> str:
    ...
    if plan_exists:
        extra = " The plan file is located at " + \
            plan_path + " if you need to reference it."
...
def build_system_prompt(
    ...
    if coordinator_mode:
        from minicode.teams.coordinator import get_coordinator_system_prompt

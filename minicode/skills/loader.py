...
from minicode.skills.parser import SkillDef, SkillParseError, VALID_NAME_RE, parse_frontmatter, parse_skill_file

log = logging.getLogger(__name__)

PROJECT_SKILLS_DIR = ".minicode/skills"
USER_SKILLS_DIR = "~/.minicode/skills"
...
class SkillLoader:
    ...
    def _scan_directory(self, path: Path, source: str) -> list[SkillDef]:
        ...
        for entry in sorted(path.iterdir()):
            try:
                ...
                log.warning("Skipping %s skill '%s': %s",
                            source, entry.name, e)
    ...
    def _load_builtins(self) -> list[SkillDef]:
        results: list[SkillDef] = []
        builtins_pkg = importlib.resources.files("minicode.skills.builtins")
        ...
        for resource in builtins_pkg.iterdir():
            ...
            try:
                ...
                from minicode.skills.parser import _validate_meta
                ...
                log.warning("Skipping builtin skill '%s': %s",
                            resource.name, e)

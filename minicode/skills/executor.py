...
from minicode.skills.parser import SkillDef, substitute_arguments
from minicode.tools import ToolRegistry

if TYPE_CHECKING:
    from minicode.agent import Agent, AgentEvent
...
class SkillExecutor:
    ...
    async def execute_fork(
        ...
        from minicode.agent import Agent as AgentClass, StreamText, LoopComplete, ErrorEvent
    ...
    def _build_fork_context(self, mode: str) -> list[Message]:
        ...
        history = self.agent._conversation.history if hasattr(
            self.agent, '_conversation') else []
        if not history:
            main_history = []
        else:

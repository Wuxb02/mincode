...
def load_tool_implementation(
    ...
    module_name = f"minicode_skill_tool_{tool_name}"
...
class SkillCustomTool(Tool):
    ...
    def get_schema(self) -> dict[str, Any]:
        input_schema = self._schema.get(
            "parameters", self._schema.get("input_schema", {}))
        return {
            "name": self.name,
...
def register_skill_tools(skill_dir: Path, registry: ToolRegistry) -> int:
    ...
    for schema in schemas:
        ...
        impl = load_tool_implementation(
            references_dir, tool_name) if references_dir.is_dir() else None

        if impl is None:
            log.warning("No implementation for tool '%s' in %s",
                        tool_name, references_dir)

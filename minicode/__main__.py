...
from minicode.config import ConfigError, load_config
from minicode.hooks import HookConfigError, HookEngine, load_hooks
from minicode.permissions import PermissionMode


def main() -> None:
    # 先确保 .minicode/ 目录存在，否则下面写 debug.log 会因目录不存在而崩溃
    Path(".minicode").mkdir(parents=True, exist_ok=True)
    logging.basicConfig(
        level=logging.INFO,
        format="%(asctime)s %(name)s %(message)s",
        filename=".minicode/debug.log",
        filemode="w",
    )

    parser = argparse.ArgumentParser(
        prog="minicode", description="minicode AI coding assistant")
    parser.add_argument(
        "--mode",
    ...
    if args.p is not None:
        ...
        asyncio.run(_run_prompt(config, permission_mode,
                    hook_engine, args.p, output_format))
        return

    # Remote 模式：启动 WebSocket 服务器，浏览器访问 http://localhost:18888
    if args.remote:
        from minicode.remote import RemoteServer
    ...
    from minicode.app import minicodeApp
    from minicode.driver import NoAltScreenDriver
...
async def _run_prompt(config, permission_mode, hook_engine, prompt: str, output_format: str = "text") -> None:
    ...
    from minicode.client import create_client, resolve_context_window
    from minicode.conversation import ConversationManager
    from minicode.memory.instructions import load_instructions
    ...
    checker = PermissionChecker(
        ...
        rule_engine=RuleEngine(
            ...
            local_rules_path=Path(work_dir) / ".minicode" /
    ...
    registry.register(TeamDeleteTool(
        team_manager=team_manager, parent_agent=agent))
    ...
    for i in range(90):
        ...
        running = {k: not t.done()
                   for k, t in task_manager._async_tasks.items()}
        completed_ids = [
        ...
        if not notes:
            ...
            if not has_running:
                print(f"[poll {i}] no running tasks, breaking",

...
def validate_providers(raw_providers: list) -> list[dict]:
    ...
    for i, entry in enumerate(raw_providers):
        ...
        missing = [f for f in (
            "name", "protocol", "base_url", "model") if f not in entry]
        if missing:
            raise ConfigError(
...
def validate_worktree(raw_wt: dict | None) -> dict:
    ...
    if not isinstance(sym, list) or not all(isinstance(s, str) for s in sym):
        raise ConfigError(
            "'worktree.symlink_directories' must be a list of strings")

    interval = raw_wt.get("stale_cleanup_interval",
                          defaults["stale_cleanup_interval"])
...
def validate_config_structure(raw: object) -> dict:
    ...
    return {
        ...
        "enable_verification_agent": validate_bool_field(
            raw.get("enable_verification_agent",
                    False), "enable_verification_agent"
        ),

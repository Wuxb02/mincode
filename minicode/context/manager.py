...
from minicode.serialization import build_messages
...
SESSION_SUBDIR = ".minicode/session/tool-results"
...
def apply_tool_result_budget(
    ...
    for msg in conversation.history:
        ...
        if total > AGGREGATE_CHAR_LIMIT:
            ranked = sorted(remaining, key=lambda tr: len(
                tr.content), reverse=True)
            for tr in ranked:
                if total <= AGGREGATE_CHAR_LIMIT:
                    break
                fp = persist_tool_result(
        ...
        if files:
            ...
            for rec in files:
                content = _truncate_by_tokens(
                    rec.content, RECOVERY_TOKENS_PER_FILE)
                ts = time.strftime(
...
async def auto_compact(
    ...
    if manual:
        ...
        soft_threshold = compute_compact_threshold(
            context_window, manual=False)
    ...
    for attempt in range(max_retries):
        try:
            from minicode.tools.base import StreamEnd, StreamEvent, TextDelta

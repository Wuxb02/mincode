...
from minicode.serialization import (
    ...
    def __init__(self, config: ProviderConfig) -> None:
        ...
        if not api_key:
            raise AuthenticationError(
                ...
                "Set it in .minicode/config.yaml or via ANTHROPIC_API_KEY env var."
            )
    ...
    async def stream(
        ...
        try:
                ...
                async for event in stream:
                    if event.type == "content_block_start":
                        ...
                        if delta_usage:
                            ...
                            v = getattr(
                                delta_usage, "cache_read_input_tokens", 0) or 0
                            if v:
                                delta_cache_read = v
                            v = getattr(
                                delta_usage, "cache_creation_input_tokens", 0) or 0
                            if v:
            ...
            retry = e.response.headers.get(
                "retry-after") if e.response else None
    ...
    def __init__(self, config: ProviderConfig) -> None:
        ...
        if not api_key:
            raise AuthenticationError(
                ...
                "Set it in .minicode/config.yaml or via OPENAI_API_KEY env var."
            )

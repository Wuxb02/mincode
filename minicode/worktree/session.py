...
def _session_path(minicode_dir: Path) -> Path:
    return minicode_dir / SESSION_FILENAME
...
def save_worktree_session(
    ...
    path = _session_path(minicode_dir)
    if session is None:
...
def load_worktree_session(minicode_dir: Path) -> WorktreeSession | None:
    path = _session_path(minicode_dir)
    if not path.exists():
        return None
    try:

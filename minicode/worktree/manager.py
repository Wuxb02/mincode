...
from minicode.worktree.changes import (
    CleanupResult,
    Changes,
    count_worktree_changes,
    has_worktree_changes,
)
from minicode.worktree.models import Worktree, WorktreeSession
from minicode.worktree.session import load_worktree_session, save_worktree_session
from minicode.worktree.setup import perform_post_creation_setup
from minicode.worktree.slug import flatten_slug, validate_slug
...
class WorktreeManager:
    def __init__(
        ...
        self.worktree_dir = worktree_dir or str(
            Path(repo_root) / ".minicode" / "worktrees"
        )
        self._minicode_dir = Path(repo_root) / ".minicode"
    ...
    def read_worktree_head_sha(wt_path: str) -> str | None:
        ...
        try:
            ...
            if commondir_file.exists():
                commondir_rel = commondir_file.read_text(
    ...
    async def enter(self, name: str) -> WorktreeSession:
        ...
        save_worktree_session(self._minicode_dir, session)
        return session
    ...
    async def exit(
        ...
        save_worktree_session(self._minicode_dir, None)
    ...
    async def _remove_worktree(self, name: str, wt: Worktree) -> None:
        ...
        if result.returncode != 0:
            log.warning("git worktree remove failed: %s",
                        result.stderr.strip())
    ...
    def restore_session(self) -> WorktreeSession | None:
        session = load_worktree_session(self._minicode_dir)
        if session is None:
            return None

...
from minicode.memory.session import SessionMeta
...
def _relative_time(meta: SessionMeta) -> str:
    ...
    dt = meta.last_active.replace(
        tzinfo=timezone.utc) if meta.last_active.tzinfo is None else meta.last_active
    delta = now - dt
...
class InlineResumeWidget(Vertical, can_focus=True):
    ...
    def _build_content(self) -> str:
        ...
        lines.append(
            "[dim]Type to search · Enter to select · Esc to cancel[/]")
        return "\n".join(lines)

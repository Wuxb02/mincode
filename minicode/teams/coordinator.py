...
def is_coordinator_mode(enable_flag: bool = False) -> bool:
    if not enable_flag:
        return False
    val = os.environ.get("minicode_COORDINATOR_MODE", "").lower()
...
def get_coordinator_system_prompt(agent_catalog: list[tuple[str, str]] | None = None) -> str:
    if agent_catalog:
        agent_lines = "\n".join(
            f"- **{name}**: {desc}" for name, desc in agent_catalog)
    else:
        agent_lines = (
    ...
    return """You are minicode, an AI assistant that orchestrates software engineering tasks across multiple workers.
...
def get_coordinator_user_context(
    ...
    if worker_tools is None:
        from minicode.agents.tool_filter import IN_PROCESS_TEAMMATE_ALLOWED_TOOLS

...
from minicode.teams.backend_detect import BackendDetectionError, detect_backend
from minicode.teams.mailbox import Mailbox, create_message
...
from minicode.teams.progress import TeammateProgress
from minicode.teams.registry import AgentNameRegistry
from minicode.teams.shared_task import SharedTaskStore
from minicode.teams.spawn_inprocess import InProcessTeammateHandle
...
class TeamManager:
    ...
    def create_team(
        ...
        log.info("Created team '%s' at %s (backend=%s)",
                 slug, team_dir, backend.value)
        return team
    ...
    def register_member(
        ...
        log.info("Registered member '%s' (agent=%s) in team '%s'",
                 member.name, member.agent_id, team_name)
    ...
    def _kill_pane(self, pane_id: str, backend_type: str) -> None:
        try:
            if backend_type == BackendType.TMUX.value:
                from minicode.teams.spawn_tmux import kill_pane
                kill_pane(pane_id)

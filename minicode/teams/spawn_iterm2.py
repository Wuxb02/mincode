...
def _run_it2(*args: str) -> str:
    ...
    if result.returncode != 0:
        raise ITermSpawnError(
            f"it2 {' '.join(args)} failed: {result.stderr.strip()}")
...
def spawn_iterm2_teammate(
    ...
    from minicode.teams.spawn_tmux import build_cli_command
    ...
    try:
        ...
        raise ITermSpawnError(
            f"Failed to spawn iTerm2 pane for {teammate_name}: {e}") from e

    log.info("Spawned iTerm2 teammate %s in session %s",
             teammate_name, session_id)

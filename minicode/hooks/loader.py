...
from minicode.hooks.conditions import ConditionParseError, parse_condition
from minicode.hooks.events import LifecycleEvent
from minicode.hooks.models import Action, Hook
...
def load_hooks(raw_hooks: list[dict] | None) -> list[Hook]:
    ...
    for i, entry in enumerate(raw_hooks):
        ...
        if not isinstance(raw_action, dict):
            raise HookConfigError(
                f"{label}: missing or invalid 'action' field")
        ...
        if not isinstance(timeout, int) or timeout <= 0:
            raise HookConfigError(
                f"{label}: timeout must be a positive integer")

...
from minicode.commands.registry import Command, CommandType
...
async def _handle_rewind(ctx) -> None:
    ...
    for i, snap in enumerate(snapshots):
        ...
        label = snap.user_text[:50] + \
            "…" if len(snap.user_text) > 50 else snap.user_text
        lines.append(
            f"  [{i + 1}] {label} ({ago}s ago, {len(snap.backups)} file(s))")

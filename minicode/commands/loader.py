"""从 .minicode/commands/ 目录加载自定义 Markdown 命令（对齐 Go 版 loader.go）。
...
def load_user_commands(work_dir: str) -> list[Command]:
    ...
    搜索路径（后者覆盖前者）：
      1. ~/.minicode/commands/
      2. <work_dir>/.minicode/commands/
    """
    dirs: list[str] = []
    home = Path.home()
    dirs.append(str(home / ".minicode" / "commands"))
    dirs.append(str(Path(work_dir) / ".minicode" / "commands"))

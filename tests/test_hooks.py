...
class TestCommandExecutor:
    ...
    async def test_normal_execution(self):
        from minicode.hooks.executors import execute_command
...
class TestPromptExecutor:
    ...
    async def test_returns_message(self):
        from minicode.hooks.executors import execute_prompt
...
class TestHttpExecutor:
    ...
    async def test_mock_request(self):
        from minicode.hooks.executors import execute_http

        action = Action(
            type="http", url="https://httpbin.org/post", body='{"test": true}')
        ctx = HookContext()
        # 用 mock 避免发起真实的网络请求
        with patch("minicode.hooks.executors.urlopen") as mock_urlopen:
...
class TestAgentExecutor:
    ...
    async def test_stub(self):
        from minicode.hooks.executors import execute_agent
...
class TestExecuteAction:
    ...
    async def test_dispatch(self):
        from minicode.hooks.executors import execute_action
...
class TestLoadHooks:
    ...
    def test_invalid_event(self):
        with pytest.raises(HookConfigError, match="invalid event"):
            load_hooks([{"event": "bad_event", "action": {
...
class TestHookEngine:
    ...
    def test_find_with_condition_filter(self):
        ...
        ctx_match = HookContext(
            event_name="post_tool_use", tool_name="WriteFile")
        assert len(engine.find_matching_hooks("post_tool_use", ctx_match)) == 1

        ctx_no_match = HookContext(
            event_name="post_tool_use", tool_name="Bash")
        assert len(engine.find_matching_hooks(
            "post_tool_use", ctx_no_match)) == 0
...
class TestAgentHookIntegration:
    ...
    async def test_pre_tool_use_reject_skips_tool(self):
        from minicode.agent import Agent, ToolResultEvent

...
from minicode.serialization import build_anthropic_messages
...
async def test_multi_step_autonomous():
    ...
    test_file = "/tmp/minicode_test_hello.txt"
    ...
    client = MockLLMClient([
        ...
        [
            ...
            ToolCallComplete("t1", "WriteFile", {
                             "file_path": "/tmp/minicode_test_hello.txt", "content": "Hello World"}),
            StreamEnd("end_turn", input_tokens=10, output_tokens=20),
        ],
        # 第 2 轮：ReadFile 进行验证
        [
            TextDelta("Verifying content."),
            ToolCallComplete("t2", "ReadFile", {
                             "file_path": "/tmp/minicode_test_hello.txt"}),
...
async def test_concurrent_batch_execution():
    ...
    client = MockLLMClient([
        [
            ToolCallComplete("t1", "ReadFile", {"file_path": "pyproject.toml"}),
            ToolCallComplete("t2", "ReadFile", {
                             "file_path": "uv.lock"}),
            StreamEnd("end_turn", input_tokens=10, output_tokens=20),
        ],
...
def test_partition_tool_calls():
    """分批逻辑会把可并发执行的调用归到同一组。"""
    from minicode.tools.base import ToolCallComplete

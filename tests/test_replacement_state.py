...
from minicode.conversation import ConversationManager, Message, ToolResultBlock


def _one_msg_conv(*results: ToolResultBlock) -> ConversationManager:
    conv = ConversationManager()
    conv.history.append(Message(role="user", content="",
...
def test_apply_does_not_mutate_conv(tmp_path: Path) -> None:
    ...
    # 并且它携带了替换后的内容
    assert api_conv.history[0].tool_results[0].content.startswith(
        PERSISTED_TAG)
...
def test_frozen_never_replaced(tmp_path: Path) -> None:
    ...
    conv = _one_msg_conv(ToolResultBlock(
        tool_use_id="t1", content="a" * quarter))
    state = create_replacement_state()
    ...
    api_t1 = next(
        tr for tr in api_conv.history[0].tool_results if tr.tool_use_id == "t1")
...
def test_reconstruct_with_inherited_parent() -> None:
    ...
    records = [
        ContentReplacementRecord(
            tool_use_id="t_child", replacement="child_preview"),
    ]

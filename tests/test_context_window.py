...
from minicode.client import resolve_context_window
from minicode.config import ProviderConfig
from minicode.validator import (
    ConfigError,
...
class TestDefaults:
    def test_claude_default(self):
        ...
        assert _provider(
            model="claude-future-99").get_context_window() == 200_000
...
class TestAutoFetch:
    ...
    async def test_fetch_success_is_cached_and_used(self):
        ...
        with patch("minicode.client.create_client", return_value=fake) as mk:
    ...
    async def test_fetch_raises_degrades_to_mapping_table(self):
        ...
        with patch("minicode.client.create_client", return_value=fake):
            # 不应抛出异常。
            await resolve_context_window(p)
        # 对 claude 回退到映射表。
    ...
    async def test_non_anthropic_provider_is_not_fetched(self):
        ...
        with patch("minicode.client.create_client") as mk:
            await resolve_context_window(p)

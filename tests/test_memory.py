...
from minicode.memory.instructions import (
    MAX_INCLUDE_DEPTH,
    load_instructions,
    process_includes,
)
...
class TestProcessIncludes:
    ...
    def test_depth_limit(self, tmp_path: Path) -> None:
        ...
        result = process_includes(
            content, tmp_path, tmp_path, depth=MAX_INCLUDE_DEPTH)
        assert result == content
...
class TestLoadInstructions:
    def test_single_layer(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        minicode_md = tmp_path / "minicode.md"
        minicode_md.write_text("project instructions", encoding="utf-8")
    ...
    def test_multi_layer_priority(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        """对齐 Go 版发现顺序：minicode.md 在前，.minicode/INSTRUCTIONS.md（legacy）在后。"""
        root_md = tmp_path / "minicode.md"
        root_md.write_text("root level", encoding="utf-8")
        dotdir = tmp_path / ".minicode"
        dotdir.mkdir()
        # Go 版不发现 .minicode/minicode.md，只发现 .minicode/INSTRUCTIONS.md（legacy）
...
class TestSessionRecord:
    ...
    def test_assistant_with_tool_uses(self) -> None:
        msg = Message(
            ...
            tool_uses=[
                ToolUseBlock(tool_use_id="t1", tool_name="ReadFile",
                             arguments={"path": "/a"})
            ],
        )
        records = SessionRecord.from_message(msg)
    ...
    def test_malformed_jsonl_returns_none(self) -> None:
        ...
        assert SessionRecord.from_jsonl(
            '{"type":"unknown","content":"x","timestamp":"2025-01-01T00:00:00"}') is None
...
class TestValidateMessageChain:
    def test_complete_chain(self) -> None:
        ...
        records = [
            ...
            SessionRecord(type=RecordType.ASSISTANT,
...
class TestRecordsToMessages:
    def test_basic_roundtrip(self) -> None:
        now = datetime.now(timezone.utc)
        records = [
            SessionRecord(type=RecordType.USER,
                          content="hello", timestamp=now),
            SessionRecord(type=RecordType.ASSISTANT,
                          content="world", timestamp=now),
        ]
    ...
    def test_system_prompt_skipped(self) -> None:
        now = datetime.now(timezone.utc)
        records = [
            SessionRecord(type=RecordType.SYSTEM_PROMPT,
                          content="system", timestamp=now),
            SessionRecord(type=RecordType.USER, content="hi", timestamp=now),
        ]
        messages = records_to_messages(records)
    ...
    def test_resume_truncates_incomplete_chain(self, tmp_path: Path) -> None:
        ...
        s.append(
            Message(
                ...
                content="checking",
                tool_uses=[
                    ToolUseBlock(tool_use_id="t1", tool_name="Bash",
...
class TestCompactBoundaryRoundTrip:
    ...
    def test_boundary_preserves_tool_pairs_in_keep(self) -> None:
        ...
        keep = [
            Message(
                ...
                content="running",
                tool_uses=[
                    ToolUseBlock(tool_use_id="t9", tool_name="Bash",
                                 arguments={"command": "ls"})
                ],
            ),
            Message(
                ...
                tool_results=[ToolResultBlock(
                    tool_use_id="t9", content="file.txt")],
            ),
    ...
    def test_resume_rebuilds_compacted_state(self, tmp_path: Path) -> None:
        ...
        summary_idx = next(i for i, c in enumerate(
            contents) if "SUMMARY OF OLD STUFF" in c)
...
class TestMemoryManager:
    ...
    def test_load_includes_memory_index(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        # 创建项目级记忆目录和文件
        project_mem_dir = tmp_path / "project" / ".minicode" / "memory"
    ...
    def test_load_all(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        # 用户级记忆
        user_mem_dir = fake_home / ".minicode" / "memory"
...
class TestConversationInjection:
    def test_inject_long_term_memory(self) -> None:
        ...
        assert "minicodeMd" in conv.history[1].content
    ...
    def test_inject_idempotent(self) -> None:
        ...
        assert sum(
            1 for m in conv.history if "<system-reminder>" in m.content) == 1
    ...
    def test_inject_instructions_only(self) -> None:
        ...
        assert "minicodeMd" in conv.history[0].content
...
class TestMemoryExtraction:
    def test_memory_types_aligned_with_go(self, tmp_path: Path) -> None:
        """验证四种记忆类型与 Go 版一致。"""
        from minicode.memory.auto_memory import VALID_TYPES, _USER_LEVEL_TYPES, _PROJECT_LEVEL_TYPES

...
from minicode.tools.base import StreamEnd, StreamEvent, TextDelta, ToolCallComplete
...
class TestPathSandbox:
    ...
    def test_temp_dir_allowed(self) -> None:
        tmp = Path(tempfile.gettempdir()) / "minicode_test.txt"
...
class TestRuleEngine:
    ...
    def test_extract_content(self) -> None:
        ...
        assert extract_content(
            "ReadFile", {"file_path": "/tmp/x.txt"}) == "/tmp/x.txt"
        assert extract_content(
            "WriteFile", {"file_path": "/tmp/y.txt", "content": "hi"}) == "/tmp/y.txt"
    ...
    def test_higher_tier_wins(self) -> None:
        ...
        engine = RuleEngine(user_rules_path=user_file,
    ...
    def test_append_local_rule(self) -> None:
        tmpdir = Path(tempfile.mkdtemp())
        local_path = tmpdir / ".minicode" / "permissions.local.yaml"
...
class TestPermissionChecker:
    ...
    def test_dangerous_command_denied(self) -> None:
        from minicode.tools.bash import Bash
        tool = Bash()
    ...
    def test_write_path_outside_sandbox_asks(self) -> None:
        from minicode.tools.write_file import WriteFile
        tool = WriteFile()
        d = self.checker.check(
            tool, {"file_path": "/etc/passwd", "content": "x"})
        assert d.effect == "ask"
        assert "沙箱" in d.reason

    def test_read_path_outside_sandbox_asks(self) -> None:
        from minicode.tools.read_file import ReadFile
        tool = ReadFile()
    ...
    def test_write_tool_asks_in_default_mode(self) -> None:
        from minicode.tools.write_file import WriteFile
        tool = WriteFile()
        d = self.checker.check(tool, {"file_path": str(
            self.tmpdir / "new.txt"), "content": "hi"})
        assert d.effect == "ask"
...
async def test_e2e_dangerous_command_blocked_loop_continues():
    ...
    agent = Agent(client, registry, "anthropic", work_dir=str(
        tmpdir), permission_checker=checker)
    ...
    ) or "拒绝" in c["tool_result"][0].output or "危险" in c["tool_result"][0].output
    assert len(c["loop"]) == 1
...
async def test_e2e_rule_allows_git():
    ...
    rules_file = tmpdir / ".minicode" / "permissions.yaml"
    rules_file.parent.mkdir(parents=True)
    rules_file.write_text(
        yaml.dump([{"rule": "Bash(git *)", "effect": "allow"}]))
...
async def test_e2e_user_denies_operation():
    ...
    client = MockLLMClient([
        [
            ToolCallComplete(
                "t1", "Bash", {"command": "npm install something"}),
            StreamEnd("end_turn", input_tokens=10, output_tokens=20),
        ],
        [

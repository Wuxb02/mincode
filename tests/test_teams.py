...
from minicode.teams.backend_detect import BackendDetectionError, detect_backend, detect_pane_backend
from minicode.teams.coordinator import (
...
class TestModels:
    ...
    def test_unique_team_name(self, tmp_dir):
        with patch("minicode.teams.models.Path.home", return_value=Path(tmp_dir)):
            name1 = unique_team_name("my-team")
            assert name1 == "my-team"
            (Path(tmp_dir) / ".minicode" / "teams" / "my-team").mkdir(parents=True)
...
class TestSharedTaskStore:
    def test_create_and_get(self, tmp_dir):
        ...
        task = store.create(title="Do something",
                            description="Details", assignee="alice")
        assert task.id == "1"
...
class TestMailbox:
    ...
    def test_broadcast(self, tmp_dir):
        ...
        mailbox.broadcast(["bob-id", "charlie-id", "alice-id"],
                          msg, exclude="alice-id")
...
class TestBackendDetect:
    ...
    def test_pane_iterm2_with_it2(self):
        ...
        with patch.dict(os.environ, env, clear=False):
            with patch("minicode.teams.backend_detect.shutil.which") as mock_which:
    ...
    def test_pane_no_backend_falls_back_to_in_process(self):
        ...
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("TMUX", None)
            os.environ.pop("TERM_PROGRAM", None)
            with patch("minicode.teams.backend_detect.shutil.which", return_value=None):
                result = detect_pane_backend()
                assert result == BackendType.IN_PROCESS
...
class TestCoordinatorMode:
    ...
    def test_enabled_with_flag_and_env(self):
        with patch.dict(os.environ, {"minicode_COORDINATOR_MODE": "1"}):
    ...
    def test_match_session_mode_switch_to_coordinator(self):
        with patch.dict(os.environ, {}, clear=False):
            ...
            assert os.environ.get("minicode_COORDINATOR_MODE") == "1"
...
class TestConfigExtensions:
    def test_teammate_mode_defaults(self):
        from minicode.config import AppConfig
...
class TestTranscript:
    ...
    def test_save_and_load(self, tmp_dir):
        ...
        from minicode.teams.transcript import load_transcript, save_transcript
    ...
    def test_load_nonexistent(self, tmp_dir):
        from minicode.teams.transcript import load_transcript
        with patch("minicode.teams.models.Path.home", return_value=Path(tmp_dir)):
...
class TestAgentCoordinatorIntegration:
    def test_normal_prompt(self):
        from minicode.prompts import build_system_prompt, IDENTITY_SECTION

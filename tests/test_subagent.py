...
from minicode.agents.parser import AgentDef, AgentParseError, parse_agent_file, parse_frontmatter
...
from minicode.agents.fork import (
    FORK_BOILERPLATE_TAG,
    ForkError,
    build_forked_messages,
)
from minicode.agents.trace import TraceManager, TraceNode
from minicode.agents.task_manager import BackgroundTask, TaskManager
from minicode.agents.notification import format_task_notification, inject_task_notifications
from minicode.conversation import ConversationManager, Message, ToolResultBlock, ToolUseBlock
...
class TestAgentLoader:
    ...
    def test_project_overrides_builtin(self, tmp_path: Path):
        agents_dir = tmp_path / ".minicode" / "agents"
...
class TestToolFilter:
    ...
    def test_disallowed_tools_in_definition(self):
        reg = make_registry("ReadFile", "EditFile",
                            "WriteFile", "Bash", "Grep")
        definition = AgentDef(
            agent_type="test",
            when_to_use="test",
    ...
    def test_background_whitelist(self):
        reg = make_registry("ReadFile", "EditFile", "WriteFile",
                            "Bash", "Grep", "Agent", "SomeOtherTool")
        definition = AgentDef(
            agent_type="test", when_to_use="test", source="builtin"
        )
...
class TestForkMode:
    ...
    def test_fork_wraps_pending_tool_use(self):
        ...
        conv.add_assistant_message(
            "Let me check",
            [ToolUseBlock(tool_use_id="tu1", tool_name="ReadFile",
...
class TestAgentToolParams:
    def test_required_fields(self):
        from minicode.tools.agent_tool import AgentToolParams

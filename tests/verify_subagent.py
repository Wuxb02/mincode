...
from minicode.conversation import ConversationManager, ToolUseBlock
...
from minicode.agents.fork import FORK_BOILERPLATE_TAG, ForkError, build_forked_messages
from minicode.agents.tool_filter import (
...
def verify_loader():
    ...
    check("security-reviewer model=haiku",
          sr is not None and sr.model == "haiku")
    check(
...
def verify_tool_filter(loader: AgentLoader):
    ...
    gp_fg = resolve_agent_tools(reg, loader.get(
        "general-purpose"), is_background=False)
    ...
    # 白名单+黑名单组合
    from minicode.agents.parser import AgentDef
    combo = AgentDef(
        agent_type="combo",
        when_to_use="test",
...
def verify_trace():
    ...
    child1 = tm.create("Explore", parent_id=root.agent_id,
                       trace_id="trace-001")
    ...
    check("get_tree 不含其他 trace", other.agent_id not in {
          n.agent_id for n in tree})
...
def verify_notification():
    ...
    from minicode.agents.task_manager import BackgroundTask
...
def verify_config():
    ...
    check("enable_verification_agent 读取成功", isinstance(
        config.enable_verification_agent, bool))
    check("enable_fork=True", config.enable_fork is True)
    check("enable_verification_agent=True",
          config.enable_verification_agent is True)
...
def verify_permission():
    print("\n== 8. BYPASS 权限模式 ==")
    from minicode.permissions.modes import PermissionMode, mode_decide

    check("BYPASS 枚举值", PermissionMode.BYPASS.value == "bypassPermissions")
    check("BYPASS read=allow", mode_decide(
        PermissionMode.BYPASS, "read") == "allow")
    check("BYPASS write=allow", mode_decide(
        PermissionMode.BYPASS, "write") == "allow")
    check("BYPASS command=allow", mode_decide(
...
def verify_agent_tool():
    ...
    from minicode.tools.agent_tool import AgentTool, AgentToolParams
